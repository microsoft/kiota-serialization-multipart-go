...
func (w *MultipartSerializationWriter) writeRawValue(value string) {
	w.writer = append(w.writer, url.QueryEscape(value))
}
func (w *MultipartSerializationWriter) writeStringValue(value string) {
	w.writeRawValue(value)
}
func (w *MultipartSerializationWriter) writePropertyName(key string) {
	w.writer = append(w.writer, url.QueryEscape(key)+"=")
}
func (w *MultipartSerializationWriter) writePropertySeparator() {
	w.writer = append(w.writer, "&")
}
func (w *MultipartSerializationWriter) trimLastPropertySeparator() {
...
func (w *MultipartSerializationWriter) WriteObjectValue(key string, item absser.Parsable, additionalValuesToMerge ...absser.Parsable) error {
	if w.depth > 0 {
		return errors.New("nested objects serialization is not supported with MultipartSerializationWriter")
	}
	w.depth++
